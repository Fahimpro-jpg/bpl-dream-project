...
setAvailableBalance(availableBalance+parseInt(p.price.split(",").join("")))
  ...
  return (
    ...
    <div className='flex font-bold'>
     ...
     <button onClick={()=>setToggle(false)} className={`btn rounded-r-2xl border-l-0 ${toggle===false?"bg-[#E7FE29]":""}`}>Selected <span>({purchasedPlayers.length})</span></button>
    </div>
    </div>

    {
      toggle === true?<Suspense fallback={<span className="loading loading-spinner loading-md"></span>}>
      <AvailablePlayers setPurchasedPlayers={setPurchasedPlayers}
